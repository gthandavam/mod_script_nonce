...
static void get_random_byte_string(unsigned char *buffer, int size)
{
    int i;

    unsigned char alphaNumericCharacters[] = "-_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";

    //What if generate_random fails ? TODO
    if( apr_generate_random_bytes(buffer, size) == APR_SUCCESS ) {
        for(i=0; i < (size-1);i++) {
            buffer[i] = ((unsigned char) (buffer[i])) >> 2;
            buffer[i] = alphaNumericCharacters[ (buffer[i]) ];
        }
    }
    buffer[size - 1] = '\0'; 
}
